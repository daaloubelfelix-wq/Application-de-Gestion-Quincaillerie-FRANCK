...
Puis ouvrir `database.py` et renseigner l'adresse IP réelle du poste serveur
dans `DB_CONFIG`.
...
├── database.py                   Connexion au serveur PostgreSQL
...
│   ├── ventes.py                  Enregistrement des ventes
...
└── ui/
    ...
    ├── dashboard_agent.py         Tableau de bord agent (vue par site)
...
- Tableau de bord agent (stock ou comptabilité) et responsable (vue consolidée
  avec sélecteur de site)
- Gestion des articles avec alertes de stock faible
- Point de vente avec génération PDF au format ticket rapide OU facture
  détaillée numérotée, TVA à 19,25% incluse
...
- Gestion des utilisateurs : création de compte, activation/désactivation,
  déverrouillage automatique à la réactivation
